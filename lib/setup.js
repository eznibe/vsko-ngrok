...
var coreDumpHandlers = {};

module.exports = setup
setup.registerHandler = function(key, fn) {
  coreDumpHandlers[key] = fn;
}
 ...
 if (opts['core-dump-on-sigint']) {
  ...
  Object.keys(coreDumpSigs).forEach(function (sig) {
    ...
    process.on(sig, function () {
      ...
      dump(sig + '.core')
      if (coreDumpHandlers[sig]) {
        debug('Calling after ' + sig + ' handler')
        coreDumpHandlers[sig]().then(function() {
          pid.cleanup()
          debug('After promise - Exiting with signal ' + +(coreDumpSigs[sig] + 128))
